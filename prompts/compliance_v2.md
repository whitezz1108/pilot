...
- the Manager's handoff: the Manager's status assessment for each target clause
  category, the decision the Manager proposed, the policy id the Manager
  applied, the Manager's evidence provenance -- the upstream claim ids it relied
  on, the source ids it inherited without opening, the paragraph ids it opened
  itself, and its stated verification basis -- the Manager's confidence, a
  reason summary, and the Manager's stated uncertainties;
...
1. Assess **each** clause category the policy names as a target, separately, on
   the evidence the handoff gives you and any passage you open yourself.
2. Apply the policy to that set of assessments to obtain the final decision.
3. Record where your evidence came from, what you did with it, and what you
   could not resolve.
...
passage of the contract. A source id names something the handoff cited. Only
report a paragraph id under `opened_paragraph_ids` if you opened that paragraph
yourself.
...
- If verification is **required**, you must consult the contract text yourself
  before concluding on a target category: search for the category, open the
  paragraphs that bear on it, and record the paragraph ids you opened under
  `opened_paragraph_ids`. A conclusion reached without opening anything does not
...
- `"upstream_only"` -- your assessment rests only on the handoff you were given.
...
- The handoff is a report by another agent, not a verified fact. Weigh it as
  such. You are not bound by the decision the Manager proposed, and you are not
  bound by the statuses it assessed: if the evidence before you supports a
  different assessment, give yours.
...
- Do not report a paragraph id you did not open. Do not report a source id or
  claim id the handoff does not contain.
...
- Assess the contract as the handoff, and any passage you opened, presents it.
  Do not speculate about what other documents might say.
...
- `evidence_provenance`: an object recording where your evidence came from. Its
  ...
  | key | value |
  | --- | --- |
  | `upstream_claim_ids` | array of strings -- claim ids from the handoff that you actually relied on |
  | `inherited_source_ids` | array of strings -- source ids you relied on because the handoff cited them, without opening them yourself |
